...
17. 양식의 라벨 셀이 ▸, ▪, ●, ○, □, ☞ 같은 기호만 있거나 매우 짧은 경우,
    ...
    - 본문 항목 번호 자리면 본문 내용 첫 줄

18. 이력서/자기소개서 양식에서 다음 원칙을 지키세요:
    - 학력/경력 반복 행은 시간 역순(현재 → 과거)으로 채우세요.
    - 이력서 페이지(인적사항/학력/자격증/기술/경력)에는 회사명/직무명/지원분야 명시 금지.
      해당 정보는 자기소개서 섹션(지원동기/직무적합성/포부)에만 들어갑니다.
    - 사용자 자료에 명시되지 않은 정보(생년월일/주소/소속 등) 추측 금지. 빈 문자열 반환.
    - 보유 기술·도구는 카테고리별 한 줄로 압축 (예: 'AI·자동화: n8n, Claude, GPT').
"""
